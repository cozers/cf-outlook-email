...
- 公开免费，无需注册 Azure 应用
- 授权时申请 Graph `Mail.ReadWrite` 权限（读 + 删）
- 支持所有 Outlook / Hotmail / Live 个人邮箱

> ℹ️ **关于刷新时的 scope：** 刷新 token 时项目按 `Mail.ReadWrite` → `Mail.Read` → `.default` 的顺序逐级尝试颗粒化 Graph scope，而**不是**直接用 `.default`。原因是 Thunderbird 这个公开应用没有静态配置任何 Graph 权限，用 `.default` 刷新会报 `AADSTS90023: No applicable permissions were found`；改用授权时实际同意过的颗粒化 scope 才能刷新成功。用自己注册的 Azure 应用不受影响。
...
可以。项目支持两种读信通道，会**自动选择**能用的那条，所以对令牌权限的兼容范围比较宽：

- **Graph 通道**：应用配置了 Graph `Mail.Read` / `Mail.ReadWrite` 权限 → 通过 Microsoft Graph API 读信（功能最全，支持附件下载）。
- **IMAP 通道**：令牌只授权了 IMAP 资源（`https://outlook.office.com/IMAP.AccessAsUser.All`）——**购买 / 领来 / 第三方刷新出来的令牌大多是这种**——则通过 IMAP over XOAUTH2 读信。

添加账号后，系统按 `auto` 策略首次访问时探测：先试 Graph，失败再试 IMAP，并把结果记住（`mail_protocol` 字段），后续直接走对的通道。「测试连接」会显示当前账号实际用的是 Graph 还是 IMAP。

| 令牌授权类型 | 测试连接 | 读邮件 | 走哪条通道 |
|----------|:--------:|:------:|:------:|
| Graph Mail.ReadWrite | ✅ | ✅（含删信 / 附件） | Graph |
| Graph Mail.Read（只读） | ✅ | ✅（不能删信） | Graph |
| 仅 IMAP（购买 / 领来的号常见） | ✅ | ✅（附件暂不支持下载） | IMAP |

> 💡 **从旧版本升级？** 之前"仅 IMAP 权限会报 401 读不了邮件"的限制已经解除。升级后**务必先跑新的数据库迁移**（见下），IMAP-only 令牌才能正常工作。
>
> ```bash
> pnpm exec wrangler d1 migrations apply outlook-email-db --remote
> pnpm exec wrangler deploy
> ```
>
> 迁移新增了 `mail_protocol` / `token_scope` 两列；已有账号默认 `auto`，首次访问自动探测协议，无需手工处理。
...
项目支持 **Graph + IMAP 双通道**，按账号令牌的实际授权自动选择（详见[可以使用其他 Client ID 吗？](#可以使用其他-client-id-吗)）：

- **Graph 通道**（Workers 用 `fetch` 调 Microsoft Graph REST API）：功能最全，支持读信、删信、附件下载。
- **IMAP 通道**（Workers 用 `connect()` 建立 TCP+TLS，XOAUTH2 认证）：用于只授权了 IMAP 资源的令牌（购买 / 领来的号常见）。支持读信、删信；**附件下载暂不支持**（会降级为不显示附件）。

暂不支持：

- **IMAP 账号的附件下载** — IMAP 附件解析尚未实现；Graph 账号附件正常
- **refresh_token 加密** — 当前明文存储在 D1（TODO）
- **邮件缓存** — 每次实时获取，不缓存到数据库
- **非微软邮箱** — 仅支持 Outlook / Hotmail / Live；Gmail / QQ / 163 等不在范围内
...
### 测试连接成功但读邮件 401（旧版本行为，已修复）

旧版本对"仅有 IMAP 权限"的令牌会出现这种情况（测试连接过，但读信报 401）。新版本已加入 **IMAP 通道**并自动探测，这类令牌现在能正常读信，无需再"重新授权切换"。若你从旧版本升级后仍遇到，请确认已跑新的数据库迁移并重新部署：
...
### 导入令牌报 `AADSTS90023: No applicable permissions were found`

令牌只授权了 IMAP 资源、没有任何 Graph 权限，而旧版本刷新时强行请求 Graph 的 `.default` scope，导致微软报此错。新版本已改为颗粒化 scope 阶梯 + IMAP 通道回退，此类令牌可正常使用。同样需要确认已跑迁移并重新部署（见上）。
